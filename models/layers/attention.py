...
class LXRTXLayer(nn.Module):
    ...
    def forward_single_stream(self, feats, attention_mask, stream):
        if stream == "text":
            att_output = self.lang_self_att(feats, attention_mask)
            inter_output = self.lang_inter(att_output)
            return self.lang_output(inter_output, att_output)
        if stream == "image":
            att_output = self.visn_self_att(feats, attention_mask)
            inter_output = self.visn_inter(att_output)
            return self.visn_output(inter_output, att_output)
